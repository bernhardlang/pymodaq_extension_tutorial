...
The method :code:`__post_init__` is called from within the dataclass' :code:`__init__` procedure. :code:`calculate_base_data` calcultates the part of the simulation which does not change with the state of the experiment. Ot should be called each time one of the parameters used in there have changed.
...
Next comes the method which is used to generate realistic spexctroscopic data.
Without light exposure detectors show some dark signal which is mostly coming from thermal noise. Its amplitude is typically proportional to the integration time. It is given in units LSB (least significant bit), i.e. in increments of the ADC. In case the signal gets too strong, the ADC signal saturate at the highest possible value (e.g. 65535 for a 16 bit ADC). The simulated signal is therefore cut to that level. Together with the data we send a time stamp.
